...
describe('deriveThinkingChoices', () => {
  test('effort "none" surfaces as off; other efforts map to same-named levels', () => {
    ...
    assert.deepEqual(
      [...deriveThinkingChoices({ efforts: ['none', 'low', 'medium', 'high', 'xhigh'] })],
  ...
  test('toggle alone does not expose off; only declared wireable off behavior does', () => {
    ...
    assert.deepEqual(
      [...deriveThinkingChoices({ efforts: ['high', 'max'], toggle: true })],
      ['high', 'max'],
    );
    assert.deepEqual(
  ...
  test('efforts without none or toggle expose no off (gpt-5 cannot disable)', () => {
    assert.deepEqual(
      [...deriveThinkingChoices({ efforts: ['minimal', 'low', 'medium', 'high'] })],
  ...
  test('effort model with no toggle and no none exposes no off (claude-opus-4-8)', () => {
    assert.deepEqual(
      [...deriveThinkingChoices({ efforts: ['low', 'medium', 'high', 'xhigh', 'max'] })],
  ...
  test('unknown effort values are dropped (not in ThinkingLevel)', () => {
    assert.deepEqual(
      [...deriveThinkingChoices({ efforts: ['low', 'turbo', 'max'] })],
      ['low', 'max'],
    );
  });
  ...
  test('choices are returned in THINKING_LEVELS display order regardless of input order', () => {
    assert.deepEqual(
      [...deriveThinkingChoices({ efforts: ['max', 'low', 'high'] })],
      ['low', 'high', 'max'],
    );
  });
});
...
describe('thinkingOptionsForModel', () => {
  ...
  test('openai gpt-5.5 exposes none/low/medium/high/xhigh; gpt-5 exposes minimal/low/medium/high', () => {
    ...
    assert.deepEqual(thinkingOptionsForModel('openai', 'gpt-5'), {
  ...
  test('zai glm-5.2 exposes high/max (not low/medium); glm-4.5-air is toggle-only', () => {
    assert.deepEqual(thinkingOptionsForModel('zai-coding-plan', 'glm-5.2'), {
      efforts: ['high', 'max'],
    });
  ...
  test('anthropic claude-opus-4-8 exposes efforts without toggle (cannot disable); fable-5 same', () => {
    assert.deepEqual(thinkingOptionsForModel('anthropic', 'claude-opus-4-8'), {
      efforts: ['low', 'medium', 'high', 'xhigh', 'max'],
    });
    assert.deepEqual(thinkingOptionsForModel('anthropic', 'claude-fable-5'), {
      efforts: ['low', 'medium', 'high', 'xhigh', 'max'],
    });
  });
  ...
  test('anthropic claude-haiku-4.5 is budget-only but can disable (toggle); sonnet-4-5 same', () => {
    ...
    assert.deepEqual(thinkingOptionsForModel('anthropic', 'claude-sonnet-4-5'), {
      toggle: true,
      offBehavior: 'anthropic-thinking-disabled',
    });
  });
  ...
  test('google gemini-3.5-flash exposes efforts; gemini-3-pro-preview exposes low/high only', () => {
    assert.deepEqual(thinkingOptionsForModel('google', 'gemini-3.5-flash'), {
      efforts: ['minimal', 'low', 'medium', 'high'],
    });
    assert.deepEqual(thinkingOptionsForModel('google', 'gemini-3-pro-preview'), {
      efforts: ['low', 'high'],
    });
  });
  ...
  test('google gemini-2.5-pro (budget-only, no toggle) has no thinking options; 2.5-flash is toggle-only', () => {
    ...
    assert.deepEqual(thinkingOptionsForModel('google', 'gemini-2.5-flash'), {
      toggle: true,
      offBehavior: 'google-thinking-budget-zero',
    });
  });
  ...
  test('Vercel Gateway keeps the exact creator/model id for Grok reasoning controls', () => {
    assert.deepEqual(thinkingOptionsForModel('vercel', 'xai/grok-4.3'), {
  ...
  test('Cloudflare Workers AI Kimi K2.6 exposes only its documented effort and off wires', () => {
    assert.deepEqual(thinkingOptionsForModel('cloudflare-workers-ai', '@cf/moonshotai/kimi-k2.6'), {
      efforts: ['low', 'medium', 'high'],
      toggle: true,
  ...
  test('Ollama Cloud reasoning models expose off/low/medium/high/max; GPT-OSS low/medium/high only; deprecated excluded', () => {
    assert.deepEqual(thinkingOptionsForModel('ollama-cloud', 'qwen3.5:397b'), {
    ...
    assert.deepEqual(thinkingOptionsForModel('ollama-cloud', 'gpt-oss:120b'), {
      efforts: ['low', 'medium', 'high'],
    });
    assert.deepEqual(
  ...
  test('claude-subscription inherits anthropic thinking options (displayMetadataOnly preserves them)', () => {
    assert.deepEqual(thinkingOptionsForModel('claude-subscription', 'claude-opus-4-8'), {
      efforts: ['low', 'medium', 'high', 'xhigh', 'max'],
    });
    assert.deepEqual(thinkingOptionsForModel('claude-subscription', 'claude-haiku-4-5'), {
      toggle: true,
      offBehavior: 'anthropic-thinking-disabled',
    });
  });
...
describe('thinkingVariantsForModel', () => {
  test('openai gpt-5.5 exposes off/low/medium/high/xhigh; gpt-5 minimal/low/medium/high; gpt-4o none', () => {
    assert.deepEqual(
      [...thinkingVariantsForModel('openai', 'gpt-5.5')],
      ['off', 'low', 'medium', 'high', 'xhigh'],
    );
    assert.deepEqual(
      [...thinkingVariantsForModel('openai', 'gpt-5')],
      ['minimal', 'low', 'medium', 'high'],
    );
  ...
  test('google gemini-3-pro-preview low/high; 3.5-flash minimal/low/medium/high; 2.5-flash off only via budget zero; 2.5-pro/2.0-flash none', () => {
    assert.deepEqual(
      [...thinkingVariantsForModel('google', 'gemini-3-pro-preview')],
      ['low', 'high'],
    );
    assert.deepEqual(
      [...thinkingVariantsForModel('google', 'gemini-3.5-flash')],
      ['minimal', 'low', 'medium', 'high'],
    );
  ...
  test('Cohere exposes off for native thinking models and no invented effort levels', () => {
    ...
    assert.deepEqual(
      [...thinkingVariantsForModel('cohere', 'command-a-reasoning-08-2025')],
      ['off'],
    );
  ...
  test('Tencent Token Plan HY 3 models expose the documented low/medium/high efforts', () => {
    ...
    assert.deepEqual(
      [...thinkingVariantsForModel('tencent-token-plan', 'hy3-preview')],
      ['low', 'medium', 'high'],
    );
  });
  ...
  test('openai-codex inherits openai gpt-5.5 thinking options', () => {
    assert.deepEqual(
      [...thinkingVariantsForModel('openai-codex', 'gpt-5.5')],
      ['off', 'low', 'medium', 'high', 'xhigh'],
    );
  });
  ...
  test('claude-subscription inherits anthropic thinking options', () => {
    assert.deepEqual(
      [...thinkingVariantsForModel('claude-subscription', 'claude-opus-4-8')],
      ['low', 'medium', 'high', 'xhigh', 'max'],
    );
    assert.deepEqual(
      [...thinkingVariantsForModel('claude-subscription', 'claude-haiku-4-5')],
      ['off'],
    );
  });
